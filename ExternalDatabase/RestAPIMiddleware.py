...
def connectToDb():
    return psycopg2.connect(host="RoadTrafficDbContainer", port = 5432, database="RoadTrafficDatabase", user="postgres", password="docker")
...
def checkIfStringIsDateTime(dateTimeStr):
    if len(dateTimeStr) != len("AAAA-LL-ZZ HH:MM:SS"):
        return False
    substr = dateTimeStr[0:4]
    if substr.isdigit() == False:
        return False
    if dateTimeStr[4] != '-':
        return False
    substr = dateTimeStr[5:7]
    if substr.isdigit() == False:
        return False
    if dateTimeStr[7] != '-':
        return False
    substr = dateTimeStr[8:10]
    if substr.isdigit() == False:
        return False
    if dateTimeStr[10] != ' ':
        return False
    substr = dateTimeStr[11:13]
    if substr.isdigit() == False:
        return False
    if dateTimeStr[13] != ':':
        return False
    substr = dateTimeStr[14:16]
    if substr.isdigit() == False:
        return False
    if dateTimeStr[16] != ':':
        return False
    substr = dateTimeStr[17:19]
    if substr.isdigit() == False:
        return False
    return True
...
def getIsAlive():
    ...
    return jsonify("Server is alive"), 200

@app.route("/api/issuereport", methods=["POST"])
def postReport():
    conn = connectToDb()
    cursor = conn.cursor()
    ...
    codeToRet = 201
    if sentJsonToServer is not None and "timestamp" in sentJsonToServer and "value" in sentJsonToServer:
        if (checkIfStringIsDateTime(sentJsonToServer["timestamp"]) and checkIfStringIsInteger(sentJsonToServer["value"])):
            timestampToInsert = sentJsonToServer["timestamp"]
            valueToInsert = sentJsonToServer["value"]
            cursor.execute("""insert into Report (issuedTime, issuedValue) values (%s, %s)""",
                (timestampToInsert, valueToInsert,))
            conn.commit()
        else:
            codeToRet = 400
